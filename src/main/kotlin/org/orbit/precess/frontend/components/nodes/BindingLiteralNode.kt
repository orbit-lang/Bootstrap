...
data class BindingLiteralNode(override val firstToken: Token, override val lastToken: Token, val ref: RefLiteralNode, val type: TermExpressionNode<*>) : DeclNode<Decl.Assignment>() {
    ...
    override fun getDecl(env: Env): DeclResult<Decl.Assignment> {
        val t = type.getExpression().infer(env)

        if (env.getRef(ref.refId) != null) return DeclResult.Failure(IType.Never("Type `${t.id}` is already defined in current context: `$env`"))

        return DeclResult.Success(Decl.Assignment(ref.refId, Expr.TypeLiteral(t.id)))
    }
}
