...
object OperatorDefInference : ITypeInference<OperatorDefNode, IMutableTypeEnvironment>, KoinComponent {
    ...
    private inline fun <reified A: AnyArrow> checkArrow(node: OperatorDefNode, fixity: OperatorFixity, delegate: AnyArrow) : A = when (delegate) {
        is Signature -> when (val a = delegate.toArrow()) {
            is A -> a
    ...
    override fun infer(node: OperatorDefNode, env: IMutableTypeEnvironment): AnyType {
        ...
        val operator = when (node.fixity) {
            OperatorFixity.Prefix -> PrefixOperator(node.symbol, node.identifierNode.identifier, checkArrow(node, node.fixity, delegate))
            OperatorFixity.Infix -> InfixOperator(node.symbol, node.identifierNode.identifier, checkArrow(node, node.fixity, delegate))
            OperatorFixity.Postfix -> PostfixOperator(node.symbol, node.identifierNode.identifier, checkArrow(node, node.fixity, delegate))
        }
