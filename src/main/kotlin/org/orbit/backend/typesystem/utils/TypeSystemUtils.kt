...
object TypeSystemUtils {
    ...
    inline fun <reified N: INode> infer(node: N, env: Env, parameters: DefinitionParameters? = null) : IType<*> {
    ...
    inline fun <reified N: INode, reified T: IType<*>> inferAs(node: N, env: Env, parameters: DefinitionParameters? = null) : T
        = infer(node, env, parameters) as T

    inline fun <reified N: INode> inferAll(nodes: List<N>, env: Env, parameters: DefinitionParameters? = null) : List<IType<*>>
        = nodes.map { infer(it, env, parameters) }

    inline fun <reified N: INode, reified T: IType<*>> inferAllAs(nodes: List<N>, env: Env, parameters: DefinitionParameters? = null) : List<T>
