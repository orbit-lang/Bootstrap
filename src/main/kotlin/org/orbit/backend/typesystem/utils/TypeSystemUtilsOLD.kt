...
import org.orbit.backend.typesystem.inference.ITypeInferenceOLD
...
object TypeInferenceUtils {
    inline fun <reified N: INode> infer(node: N, env: ITypeEnvironment, parameters: DefinitionParameters? = null) : AnyType {
...
object TypeSystemUtilsOLD : KoinComponent {
    ...
    inline fun <reified N: INode> infer(node: N, env: Env, parameters: DefinitionParameters? = null) : AnyType {
        val inference = KoinPlatformTools.defaultContext().get().get<ITypeInferenceOLD<N>>(named("infer${node::class.java.simpleName}")) { parameters ?: parametersOf() }
