...
data class Context(
    ...
    fun apply(inferenceUtil: InferenceUtil, substitutions: List<Substitution>) : InferenceUtil {
        return substitutions.fold(inferenceUtil) { acc, next -> next.apply(acc) }
    }
    ...
    fun check(inferenceUtil: InferenceUtil, given: List<TypeComponent>) : Boolean {
        ...
        val nConstraints = constraints.map {
            subs.fold(it) { acc, next -> acc.substitute(next.typeVariable, next.type) }
        }
        ...
        return nConstraints.all { it.check(ctx) }
    }
